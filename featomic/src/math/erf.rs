...
//! under MIT license.
//!
//! See <https://github.com/statrs-dev/statrs/blob/5411ba74b427b992dd1410d699052a0b41dc2b5c/src/function/erf.rs>
...
fn erf_impl(z: f64, inv: bool) -> f64 {
    ...
    let result = if z < 0.5 {
        if z < 1e-10 {
            ...
            z * 1.125
                + z * evaluate_polynomial(z, ERF_IMPL_AN) / evaluate_polynomial(z, ERF_IMPL_AD)
        }
    } else if z < 110.0 {
        let (r, b) = if z < 0.75 {
            (
                evaluate_polynomial(z - 0.5, ERF_IMPL_BN)
                    / evaluate_polynomial(z - 0.5, ERF_IMPL_BD),
                0.3440242112,
            )
        } else if z < 1.25 {
            (
                evaluate_polynomial(z - 0.75, ERF_IMPL_CN)
                    / evaluate_polynomial(z - 0.75, ERF_IMPL_CD),
                0.419990927,
            )
        } else if z < 2.25 {
            (
                evaluate_polynomial(z - 1.25, ERF_IMPL_DN)
                    / evaluate_polynomial(z - 1.25, ERF_IMPL_DD),
                0.4898625016,
            )
        } else if z < 3.5 {
            (
                evaluate_polynomial(z - 2.25, ERF_IMPL_EN)
                    / evaluate_polynomial(z - 2.25, ERF_IMPL_ED),
                0.5317370892,
            )
        } else if z < 5.25 {
            (
                evaluate_polynomial(z - 3.5, ERF_IMPL_FN)
                    / evaluate_polynomial(z - 3.5, ERF_IMPL_FD),
                0.5489973426,
            )
        } else if z < 8.0 {
            (
                evaluate_polynomial(z - 5.25, ERF_IMPL_GN)
                    / evaluate_polynomial(z - 5.25, ERF_IMPL_GD),
                0.5571740866,
            )
        } else if z < 11.5 {
            (
                evaluate_polynomial(z - 8.0, ERF_IMPL_HN)
                    / evaluate_polynomial(z - 8.0, ERF_IMPL_HD),
                0.5609807968,
            )
        } else if z < 17.0 {
            (
                evaluate_polynomial(z - 11.5, ERF_IMPL_IN)
                    / evaluate_polynomial(z - 11.5, ERF_IMPL_ID),
                0.5626493692,
            )
        } else if z < 24.0 {
            (
                evaluate_polynomial(z - 17.0, ERF_IMPL_JN)
                    / evaluate_polynomial(z - 17.0, ERF_IMPL_JD),
                0.5634598136,
            )
        } else if z < 38.0 {
            (
                evaluate_polynomial(z - 24.0, ERF_IMPL_KN)
                    / evaluate_polynomial(z - 24.0, ERF_IMPL_KD),
                0.5638477802,
            )
        } else if z < 60.0 {
            (
                evaluate_polynomial(z - 38.0, ERF_IMPL_LN)
                    / evaluate_polynomial(z - 38.0, ERF_IMPL_LD),
                0.5640528202,
            )
        } else if z < 85.0 {
            (
                evaluate_polynomial(z - 60.0, ERF_IMPL_MN)
                    / evaluate_polynomial(z - 60.0, ERF_IMPL_MD),
                0.5641309023,
            )
        } else {
            (
                evaluate_polynomial(z - 85.0, ERF_IMPL_NN)
                    / evaluate_polynomial(z - 85.0, ERF_IMPL_ND),
                0.5641584396,
            )
        };
        let g = (-z * z).exp() / z;
...
//         let g = p * (p + 10.0);
//         let r = evaluate_polynomial(p, ERF_INV_IMPL_AN) / evaluate_polynomial(p, ERF_INV_IMPL_AD);
//         g * y + g * r
//     } else if q >= 0.25 {
...
//         let xs = q - 0.25;
//         let r =
//             evaluate_polynomial(xs, ERF_INV_IMPL_BN) / evaluate_polynomial(xs, ERF_INV_IMPL_BD);
//         g / (y + r)
//     } else {
//         let x = (-q.ln()).sqrt();
//         if x < 3.0 {
...
//             let xs = x - 1.125;
//             let r = evaluate_polynomial(xs, ERF_INV_IMPL_CN)
//                 / evaluate_polynomial(xs, ERF_INV_IMPL_CD);
//             y * x + r * x
//         } else if x < 6.0 {
//             let y = 0.93995571136474609375;
//             let xs = x - 3.0;
//             let r = evaluate_polynomial(xs, ERF_INV_IMPL_DN)
//                 / evaluate_polynomial(xs, ERF_INV_IMPL_DD);
//             y * x + r * x
//         } else if x < 18.0 {
//             let y = 0.98362827301025390625;
//             let xs = x - 6.0;
//             let r = evaluate_polynomial(xs, ERF_INV_IMPL_EN)
//                 / evaluate_polynomial(xs, ERF_INV_IMPL_ED);
//             y * x + r * x
//         } else if x < 44.0 {
//             let y = 0.99714565277099609375;
//             let xs = x - 18.0;
//             let r = evaluate_polynomial(xs, ERF_INV_IMPL_FN)
//                 / evaluate_polynomial(xs, ERF_INV_IMPL_FD);
//             y * x + r * x
//         } else {
//             let y = 0.99941349029541015625;
//             let xs = x - 44.0;
//             let r = evaluate_polynomial(xs, ERF_INV_IMPL_GN)
//                 / evaluate_polynomial(xs, ERF_INV_IMPL_GD);
//             y * x + r * x
//         }
//     };
//     s * result
...
/// Evaluates a polynomial at `z` where `coeff` are the coefficients to a
/// polynomial of order `k` where `k` is the length of `coeff` and the
/// coefficient to the `k`th power is the `k`th element in coeff. E.g. [3,-1,2]
/// equates to `2z^2 - z + 3`
///
/// # Remarks
///
/// Returns 0 for a 0 length coefficient slice
pub fn evaluate_polynomial(z: f64, coeff: &[f64]) -> f64 {
    let n = coeff.len();
    if n == 0 {
        return 0.0;
    }
...
mod tests {
    ...
    fn test_polynomial() {
        let empty: [f64; 0] = [];
        assert_eq!(evaluate_polynomial(2.0, &empty), 0.0);

        let zero = [0.0];
        assert_eq!(evaluate_polynomial(2.0, &zero), 0.0);

        let mut coeff = [1.0, 0.0, 5.0];
        assert_eq!(evaluate_polynomial(2.0, &coeff), 21.0);

        coeff = [-5.0, -2.0, 3.0];
        assert_eq!(evaluate_polynomial(2.0, &coeff), 3.0);
        assert_eq!(evaluate_polynomial(-2.0, &coeff), 11.0);

        let large_coeff = [-1.35e3, 2.5e2, 8.0, -4.0, 1e2, 3.0];
        assert_eq!(evaluate_polynomial(5.0, &large_coeff), 71475.0);
        assert_eq!(evaluate_polynomial(-5.0, &large_coeff), 51225.0);

        coeff = [f64::INFINITY, -2.0, 3.0];
        assert_eq!(evaluate_polynomial(2.0, &coeff), f64::INFINITY);
        assert_eq!(evaluate_polynomial(-2.0, &coeff), f64::INFINITY);

        coeff = [f64::NEG_INFINITY, -2.0, 3.0];
        assert_eq!(evaluate_polynomial(2.0, &coeff), f64::NEG_INFINITY);
        assert_eq!(evaluate_polynomial(-2.0, &coeff), f64::NEG_INFINITY);

        coeff = [f64::NAN, -2.0, 3.0];
        assert!(evaluate_polynomial(2.0, &coeff).is_nan());
        assert!(evaluate_polynomial(-2.0, &coeff).is_nan());
    }
    ...
    fn test_erf() {
        ...
        assert_relative_eq!(erf(-1.0), -0.8427007929497149, max_relative=1e-11);
        assert_eq!(erf(0.0), 0.0);
        assert_eq!(erf(1e-15), 1.1283791670955127e-15);
        assert_eq!(erf(0.1), 0.1124629160182849);
        assert_relative_eq!(erf(0.2), 0.22270258921047847, max_relative=1e-16);
        assert_eq!(erf(0.3), 0.3286267594591274);
        assert_eq!(erf(0.4), 0.42839235504666845);
        assert_relative_eq!(erf(0.5), 0.5204998778130465, max_relative=1e-9);
        assert_relative_eq!(erf(1.0), 0.8427007929497149, max_relative=1e-11);
        assert_relative_eq!(erf(1.5), 0.9661051464753108, max_relative=1e-11);
        assert_relative_eq!(erf(2.0), 0.9953222650189527, max_relative=1e-11);
        assert_relative_eq!(erf(2.5), 0.999593047982555, max_relative=1e-13);
        assert_relative_eq!(erf(3.0), 0.9999779095030014, max_relative=1e-11);
        assert_eq!(erf(4.0), 0.9999999845827421);
        assert_eq!(erf(5.0), 0.9999999999984626);
        assert_eq!(erf(6.0), 0.9999999999999999784802632);
    ...
    fn test_erfc() {
        ...
        assert_relative_eq!(erfc(-1.0), 1.8427007929497148, max_relative=1e-11);
        assert_eq!(erfc(0.0), 1.0);
        assert_relative_eq!(erfc(0.1), 0.887537083981715, max_relative=1e-15);
        assert_eq!(erfc(0.2), 0.7772974107895215);
        assert_eq!(erfc(0.3), 0.6713732405408726);
        assert_relative_eq!(erfc(0.4), 0.5716076449533315, max_relative=1e-15);
        assert_relative_eq!(erfc(0.5), 0.4795001221869535, max_relative=1e-9);
        assert_relative_eq!(erfc(1.0), 0.15729920705028513, max_relative=1e-9);
        assert_relative_eq!(erfc(1.5), 0.033894853524689274, max_relative=1e-9);
        assert_relative_eq!(erfc(2.0), 0.004677734981047266, max_relative=1e-9);
        assert_relative_eq!(erfc(2.5), 0.0004069520174449589, max_relative=1e-9);
        assert_relative_eq!(erfc(3.0), 0.00002209049699858544, max_relative=1e-9);
        assert_relative_eq!(erfc(4.0), 0.00000001541725790028002, max_relative=1e-18);
        assert_relative_eq!(erfc(5.0), 1.5374597944280348e-12, max_relative=1e-22);
        assert_relative_eq!(erfc(6.0), 2.1519736712498913e-17, max_relative=1e-26);
        assert_relative_eq!(erfc(10.0), 2.088487583762545e-45, max_relative=1e-55);
        assert_relative_eq!(erfc(15.0), 7.212994172451207e-100, max_relative=1e-109);
        assert_relative_eq!(erfc(20.0), 5.395865611607901e-176, max_relative=1e-186);
        assert_eq!(erfc(30.0), 0.0);
        assert_eq!(erfc(50.0), 0.0);
        assert_eq!(erfc(80.0), 0.0);
    ...
    //     assert_relative_eq!(erf_inv(1e-15), 8.86226925452758e-16, max_relative=1e-30);
    //     assert_eq!(erf_inv(0.1), 8.885599049425769e-2);
    //     assert_relative_eq!(erf_inv(0.2), 0.17914345462129166, max_relative=1e-15);
    //     assert_eq!(erf_inv(0.3), 0.2724627147267544);
    //     assert_eq!(erf_inv(0.4), 0.37080715859355795);
    //     assert_eq!(erf_inv(0.5), 0.4769362762044699);
    ...
    //     assert_relative_eq!(erfc_inv(1e-100), 15.065574702593, max_relative=1e-11);
    //     assert_relative_eq!(erfc_inv(1e-30), 8.1486162231699, max_relative=1e-12);
    //     assert_relative_eq!(erfc_inv(1e-20), 6.6015806223551, max_relative=1e-13);
    //     assert_relative_eq!(erfc_inv(1e-10), 4.572824958544925, max_relative=1e-7);
    //     assert_relative_eq!(erfc_inv(1e-5), 3.123413274341571, max_relative=1e-11);
    //     assert_relative_eq!(erfc_inv(0.1), 1.1630871536766743, max_relative=1e-14);
    //     assert_relative_eq!(erfc_inv(0.2), 0.9061938024368233, max_relative=1e-15);
    //     assert_eq!(erfc_inv(0.5), 0.4769362762044699);
    //     assert_eq!(erfc_inv(1.0), 0.0);
    //     assert_eq!(erfc_inv(1.5), -0.4769362762044699);
