//! utility functions to run rascaline-torch tests from Cargo
...
#[path = "../../../rascaline-c-api/tests/utils/mod.rs"]
mod core_utils;
...
pub fn setup_pytorch(build_dir: PathBuf) -> PathBuf {
    ...
    let torch_version = std::env::var("RASCALINE_TORCH_TEST_VERSION").unwrap_or("2.3.*".into());
    let status = Command::new(&python)
        .arg("-m")
        .arg("pip")
        .arg("install")
...
/// Build rascaline in `build_dir`, and return the installation prefix
pub fn setup_rascaline(build_dir: PathBuf) -> PathBuf {
    let mut rascaline_source_dir = PathBuf::from(std::env::var("CARGO_MANIFEST_DIR").unwrap());
    rascaline_source_dir.extend(["..", "rascaline-c-api"]);

    // configure cmake for rascaline
    let mut cmake_config = cmake_config(&rascaline_source_dir, &build_dir);
    ...
    assert!(status.success(), "failed to run rascaline cmake configuration");

    // build and install rascaline
    ...
    assert!(status.success(), "failed to run rascaline cmake build");
