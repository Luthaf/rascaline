//! utility functions to run featomic-torch tests from Cargo
...
pub fn setup_featomic(build_dir: PathBuf) -> PathBuf {
    ...
    cmake_config.arg("-DFEATOMIC_ENABLE_CHEMFILES=OFF");

    let status = cmake_config.status().expect("could not run cmake");
    assert!(status.success(), "failed to run featomic cmake configuration");

    // build and install featomic
    ...
    assert!(status.success(), "failed to run featomic cmake build");
