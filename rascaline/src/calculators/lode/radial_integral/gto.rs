...
impl LodeRadialIntegral for LodeRadialIntegralGto {
    ...
    fn get_center_contribution(&self, density: DensityKind) -> Result<Array1<f64>, Error> {
        ...
        let (atomic_gaussian_width, potential_exponent) = match density {
            DensityKind::LongRangeGaussian { width, exponent } => {
                (width, exponent as f64)
            }
            _ => {
                return Err(Error::InvalidParameter(
                    "Only 'LongRangeGaussian' density is supported in LODE".into()
                ));
            }
        };
        ...
        if potential_exponent == 0.0 {
            ...
            for n in 0..radial_size {
                let alpha = 0.5
                    * (1.0 / (atomic_gaussian_width * atomic_gaussian_width)
            ...
            let factor = 2.0 * f64::sqrt(4.0 * std::f64::consts::PI)
                / gamma(potential_exponent / 2.0)
                / potential_exponent;

            for n in 0..radial_size {
                let s = atomic_gaussian_width / self.gto_gaussian_widths[n];
                let hyparg = 1.0 / (1.0 + s * s);

                contrib[n] = factor
                    * f64::powf(2.0, (1.0 + n as f64 - potential_exponent) / 2.0)
                    * atomic_gaussian_width.powi(3 + n as i32 - potential_exponent as i32)
                    * gamma(n_eff[n])
                    * hyp2f1(1.0, n_eff[n], (potential_exponent + 2.0) / 2.0, hyparg)
                    * hyparg.powf(n_eff[n]);
            }
        }
...
mod tests {
    ...
    fn central_atom_contribution() {
        ...
        for (i, &p) in potential_exponents.iter().enumerate() {
            ...
            let density = DensityKind::LongRangeGaussian { width: 1.0, exponent: p };
