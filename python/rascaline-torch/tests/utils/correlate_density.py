...
from rascaline.torch.utils.clebsch_gordan._cg_cache import ClebschGordanReal
from rascaline.torch.utils.clebsch_gordan._clebsch_gordan import _standardize_keys
...
#        norm += torch.sum(
#            [
#                torch.linalg.norm(block.values[0, m, :]) ** 2
...
#            names=["spherical_harmonics_l"], values=torch.array([0]).reshape(-1, 1)
#        ),
#    )
...
#    assert torch.allclose(norm_nu1, norm_nux)
#
#    # But with sorting the l list we should get a different norm
#    assert not torch.allclose(norm_nu1, norm_nux_sorted_l)
#
#
## ============ Test CG cache  ============
#
...
#        diag_mask = torch.zeros(dot_product.shape, dtype=torch.bool_)
#        diag_mask[torch.diag_indices(len(dot_product))] = True
#        assert torch.allclose(
#            dot_product[~diag_mask], torch.zeros(dot_product.shape)[~diag_mask]
#        )
#        assert torch.allclose(dot_product[diag_mask], dot_product[diag_mask][0])
#
...
#    dot_product = torch.zeros((l1l2_dim, l1l2_dim))
...
#    diag_mask[torch.diag_indices(len(dot_product))] = True
#
#    assert torch.allclose(dot_product[~diag_mask], torch.zeros(dot_product.shape)[~diag_mask])
#    assert torch.allclose(dot_product[diag_mask], dot_product[diag_mask][0])
#
#
#@pytest.mark.skipif(
...
#    [
#        None,
#        Labels(names=["spherical_harmonics_l"], values=torch.array([1, 3]).reshape(-1, 1)),
#    ],
#)
...
#        assert torch.all(
#            [
#                angular in torch.arange(SPHEX_HYPERS["max_angular"] * 2 + 1)
#                for angular in torch.unique(nu_2.keys.column("spherical_harmonics_l"))
#            ]
#        )
#
#    else:
#        assert torch.all(
#            torch.sort(torch.unique(nu_2.keys.column("spherical_harmonics_l")))
#            == torch.sort(selected_keys.column("spherical_harmonics_l"))
#        )
