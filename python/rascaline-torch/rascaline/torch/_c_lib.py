...
if not version_compatible(torch.__version__, BUILD_TORCH_VERSION):
    raise ImportError(
        f"Trying to load rascaline-torch with torch v{torch.__version__}, "
        f"but it was compiled against torch v{BUILD_TORCH_VERSION}, which "
        "is not ABI compatible"
    )

if not version_compatible(rascaline.__version__, BUILD_RASCALINE_VERSION):
    raise ImportError(
        f"Trying to load rascaline-torch with rascaline v{rascaline.__version__}, "
        f"but it was compiled against rascaline v{BUILD_RASCALINE_VERSION}, which "
        "is not ABI compatible"
    )
...
def _lib_path():
    if sys.platform.startswith("darwin"):
        path = os.path.join(_HERE, "lib", "librascaline_torch.dylib")
        windows = False
    elif sys.platform.startswith("linux"):
        path = os.path.join(_HERE, "lib", "librascaline_torch.so")
        windows = False
    elif sys.platform.startswith("win"):
        path = os.path.join(_HERE, "bin", "rascaline_torch.dll")
        windows = True
    else:
    ...
    raise ImportError("Could not find rascaline_torch shared library at " + path)
