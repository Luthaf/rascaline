...
def test_soap_spliner():
    ...
    hypers = {
        "cutoff": rascaline.cutoff.Cutoff(
            radius=cutoff,
            smoothing=rascaline.cutoff.Step(),
        ),
        "density": rascaline.density.Gaussian(width=0.6),
        "basis": rascaline.basis.TensorProduct(
            max_angular=6,
            radial=rascaline.basis.Gto(max_radial=6, radius=cutoff),
    ...
    spliner = rascaline.splines.SoapSpliner(**hypers)
...
def test_lode_spliner(exponent):
    ...
    hypers = {
        "density": rascaline.density.SmearedPowerLaw(smearing=0.8, exponent=exponent),
        "basis": rascaline.basis.TensorProduct(
            max_angular=4,
            radial=rascaline.basis.Gto(max_radial=4, radius=2),
        ),
    }

    spliner = rascaline.splines.LodeSpliner(**hypers)
...
def test_lode_center_contribution():
    ...
    smearing = 1.2
    hypers = {
        "density": rascaline.density.SmearedPowerLaw(smearing=smearing, exponent=0),
        "basis": rascaline.basis.TensorProduct(
            max_angular=4,
            radial=rascaline.basis.Gto(max_radial=max_radial, radius=gto_radius),
            # We choose an accuracy that is lower then the default one (1e-8)
            # to limit the time taken by this test.
            spline_accuracy=1e-3,
        ),
...
def test_custom_radial_integral():
    hypers = {
        ...
        "density": rascaline.density.Gaussian(width=1.2),
        "basis": rascaline.basis.TensorProduct(
            max_angular=2,
            radial=rascaline.basis.Gto(max_radial=3, radius=2.0),
        ),
    }

    n_spline_points = 3
    spliner_gaussian = rascaline.splines.SoapSpliner(
    ...
    class NotAGaussian(rascaline.density.AtomicDensity):
    ...
    spliner_custom = rascaline.splines.SoapSpliner(
        **hypers, n_spline_points=n_spline_points
    )
