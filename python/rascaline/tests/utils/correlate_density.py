...
def test_clebsch_gordan_orthogonality(cg_cache_dense, l1, l2):
    ...
    for lam in range(lam_min, lam_max):
        ...
        diag_mask = _dispatch.zeros_like(dot_product.shape, bool_like)
        diag_indices = (
            _dispatch.arange_like(int64_like, 0, len(dot_product)),
            _dispatch.arange_like(int64_like, 0, len(dot_product)),
        )
        diag_mask[diag_indices] = True
        assert _dispatch.allclose(
            dot_product[~diag_mask],
            _dispatch.zeros_like(dot_product.shape, float64_like)[~diag_mask],
            rtol=1e-05,
            atol=1e-08,
        )
        assert _dispatch.allclose(
            dot_product[diag_mask], dot_product[diag_mask][0], rtol=1e-05, atol=1e-08
        )
    ...
    dot_product = _dispatch.zeros_like((l1l2_dim, l1l2_dim), float64_like)
