...
# function, that defined the type of density under consideration. For a given central
# atom :math:`i` in the system, the total density function
# :math:`\rho_i(\boldsymbol{r})` around is then defined as
# :math:`\rho_i(\boldsymbol{r}) = \sum_{j} g(\boldsymbol{r} - \boldsymbol{r}_{ij})`.

# Atomic densities are represented as different child class of
# :py:class:`rascaline.utils.AtomicDensityBase`:
# :py:class:`rascaline.utils.DeltaDensity`, :py:class:`rascaline.utils.GaussianDensity`,
# and :py:class:`rascaline.utils.LodeDensity` are provided, and you can implement your
# own by defining a new class.
...
#     :param atomic_gaussian_width: Width of the atom-centered gaussian used to create
#         the atomic density
#     """
...
#         connected to SOAP or physics-based quantities: p=0 uses Gaussian densities as
#         in SOAP, p=1 uses 1/r Coulomb like densities, p=6 uses 1/r^6 dispersion like
#         densities.
#     """
...
#             # Even though we use `np.where` to apply the `_short_range` method for
#             # small `x`, the `_long_range` method will also evaluated for small `x`
#             # and issueing RuntimeWarnings. We filter these warnings to avoid that
