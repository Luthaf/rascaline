...
# Radial basis functions :math:`R_{nl}(\boldsymbol{r})` are besides :ref:`atomic
# densities <python-atomic-density>` :math:`\rho_i` the central ingredients to compute
# spherical expansion coefficients :math:`\langle anlm\vert\rho_i\rangle`. Radial basis
# functions, define how which the atomic density is projected. To be more precise,
# the actual basis functions are of
...
#     The class provides methods to evaluate the radial basis :math:`R_{nl}(r)` as well
#     as its (numerical) derivative with respect to positions :math:`r`.
...
#         defined in a child class, a numerical derivative based on finite differences
#         of ``integrand_positions`` will be used instead.
...
#         # choosing infinity leads to problems when calculating the radial integral
#         # with `quad`!
...
#     where :math:`n` runs from :math:`0,1,...,n_\mathrm{max}-1`. These capture
#     precisely the radial dependence if we compute the Taylor expansion of a generic
#     function defined in 3D space.
...
#         `Scipy Cookbook
#          <https://scipy-cookbook.readthedocs.io/items/SphericalBesselZeros.html>`_.
