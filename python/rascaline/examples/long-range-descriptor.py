...
# exponentially like a :py:class:`Gaussian <rascaline.density.Gaussian>` density and is
# therefore suited to describe long-range interactions between atoms.


density = rascaline.density.SmearedPowerLaw(smearing=1.2, exponent=3)
...
gaussian_density = rascaline.density.Gaussian(width=density.smearing)
...
# <rascaline.density.Gaussian>` density decays exponentially and is therefore not suited
# for long-range descriptors.

# %%
#
# As a projection basis, we don't use the usual :py:class:`Gto <rascaline.basis.Gto>`
# which is commonly used for short range descriptors. Instead, we select the
# :py:class:`Monomials <rascaline.basis.Monomials>` which is the optimal radial basis
...
for angular in range(2):
    by_angular[angular] = rascaline.basis.Monomials(
...
real_space_spliner = rascaline.splines.SoapSpliner(
    ...
    cutoff=rascaline.cutoff.Cutoff(radius=cutoff, smoothing=None),
    basis=basis,
    density=density,
)
real_space_hypers = real_space_spliner.get_hypers()

fourier_space_spliner = rascaline.splines.LodeSpliner(
    basis=basis,
    density=density,
)
...
# :py:class:`rascaline.SphericalExpansion` for your machine learning models.
#
