...
# #
# # To start this construction we choose a high potential exponent to emulate the
# # rapidly decaying LODE density and mimic the polar-polar interactions of water.
...
# # Instead, we select the :py:class:`MonomialBasis <rascaline.utils.MonomialBasis>`
# # which is the optimal radial basis for the LODE descriptor as discussed in
# # `Huguenin-Dumittan et al. <https://doi.org/10.1021/acs.jpclett.3c02375>`_
...
# #   load them with :py:func:`json.load` later without recalculating them. Saving them
# #   is especially useful if the spline calculations are expensive, i.e., if you
# #   increase the ``spline_accuracy``.
# #
...
# # visualize our result. Since we have to perform the densify operation several times
# # in this how-to, we define a helper function ``densify_tensormap``.
