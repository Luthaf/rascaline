...
# truncations approaches. The basis can be truncated in the "traditional"  way, using
# all values below a limit in the angular and radial direction; or using a "ragged
...
# radial integral corresponding to the LE basis. An detailed how-to guide how to
# construct radial integrals is given in :ref:`userdoc-how-to-splined-radial-integral`.
# """
...
# # basis functions with ``l <= max_angular`` and ``n < max_radial``. The basis
# # functions are the solution of a radial Laplacian eigenvalue problem (spherical
# # Bessel functions).
...
# # %%
# #
# # This calculator defaults to the "traditional" basis function selection, so we have
# # the same maximal ``n`` value for all ``l``.
...
# # treat on the same footings the radial and angular dimension, and select all
# # functions with a mean Laplacian below a certain threshold. This is similar to the
# # common practice in plane-wave electronic-structure methods to use a kinetic energy
# # cutoff where :math:`k_x^2 + k_y^2 + k_z^2 < k_\text{max}^2`
...
# # %%
# #
# # Comparing this eigenvalues threshold with the one based on a square selection, we
# # see that the eigenvalues threshold leads to a gradual decrease of ``max_radial``
# # for high ``l`` values
